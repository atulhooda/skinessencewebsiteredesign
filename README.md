...
- The entry popup (`components/lead/`, copy and timing in `content/pages/popup.json`) opens a few seconds after any page loads, except `/location`. This is the pattern Google treats as an intrusive interstitial on mobile; watch Search Console after launch and raise the delay, raise the repeat interval or set `enabled: false` if rankings move
- Analytics loads from the clinic's own subdomain, `https://track.skinessence2017.com`, which only accepts requests from `https://skinessence2017.com`. Until that domain serves this site, every page on the staging URL logs a CORS error and sends no data, which also costs a Lighthouse best-practices point. Ask the vendor to allowlist the staging domain if the client wants to test the dashboard before launch
- Funnel Agent analytics is live on every page (`components/layout/FunnelTracking.tsx`). It captures the booking form: name, phone, email, the treatment/concern and clinic dropdowns, and the WhatsApp consent box. Free text is not sent. Have the clinic confirm they have a data-processing agreement with the vendor, since a treatment enquiry tied to a name and number is health-related personal data. `NEXT_PUBLIC_FUNNEL_KEY=""` switches it off; `data-fa-no-capture` on a `<form>` exempts that form
- The privacy policy now discloses the analytics service and the WhatsApp opt-in (`content/pages/privacy-policy.json`, updated 23 Sep 2026). Keep it in step with whatever the vendor actually receives
