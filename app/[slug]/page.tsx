 ...
 * location landing pages (/dermatologist-in-kalyani-nagar) and doctor
 * profiles (/dr-daksha-patel). Static folders such as /about take precedence.
 */
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (doctor) return buildMetadata({ title: doctorTitle(doctor), description: doctor.metaDescription, path: routes.doctorProfile(slug) });
  return {};
}
...
export default async function TopLevelContentPage({ params }: { params: Promise<Params> }) {
  ...
  const doctor = getDoctor(slug);
  if (doctor) return <DoctorPage doctor={doctor} />;
  notFound();
}
