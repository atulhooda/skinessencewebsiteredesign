...
/** Homepage treatments: named rectangles with a small image header, four across on desktop. */
  ...
  return (
    <SectionCard id={id} headingId={`${id}-heading`} tone={tone}>
      <Container>
        ...
        <ul className="mt-10 grid grid-cols-2 gap-3 md:mt-16 lg:grid-cols-4">
          {tiles.map((tile) => (
            <li key={tile.href + tile.name}>
              <TreatmentTile treatment={tile.treatment} name={tile.name} href={tile.href} image={tile.image} label={categoryName(tile.treatment.category)} withImage />
            </li>
          ))}
        </ul>
      </Container>
